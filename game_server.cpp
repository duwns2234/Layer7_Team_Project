...
int main(int argc, char *argv[]) {
    ...
    sockinfo.sin_port = htons(7777); // 서버 포트
    ...
    while (p1 != 0 && p2 != 0) {
        char buf1[100], buf2[100]; //입력받을 버퍼 생성

        if (switch_order) { // 클라1과 클라2 번갈아가면서 받아주기
            recv(clientsock1, buf1, sizeof(buf1), 0); // 버퍼에 키값 입력받기
            switch_order = 0;
            playerInput = (char)buf1;
        }
        else {
            recv(clientsock2, buf2, sizeof(buf2), 0);
            switch_order = 1;
            playerInput = (char)buf1;
        }
